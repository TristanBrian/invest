# Stripe Integration - Architecture & Flow Diagrams

## 🏗️ System Architecture

\`\`\`
┌─────────────────────────────────────────────────────────────┐
│                    YOUR APPLICATION                         │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Frontend (Product Display, Checkout Button, etc.)        │
│         │                                                  │
│         ├──> Redirect to Stripe Checkout                  │
│         │                                                  │
│         └──> Process Payment                              │
│                                                             │
├─────────────────────────────────────────────────────────────┤
│                    API ENDPOINTS                           │
│                                                             │
│  /api/stripe/checkout ─────┐                              │
│  /api/stripe/products      ├─> Stripe Client              │
│  /api/stripe/connect/*     │   (Validated & Secure)       │
│  /api/webhooks/stripe ─────┘                              │
│                                                             │
├─────────────────────────────────────────────────────────────┤
│                    ENVIRONMENT VARIABLES                    │
│                                                             │
│  STRIPE_SECRET_KEY          ✅ Secured                    │
│  STRIPE_PUBLISHABLE_KEY     ✅ Secured                    │
│  STRIPE_WEBHOOK_SECRET      ✅ Secured                    │
│  NEXT_PUBLIC_APP_URL        ✅ Secured                    │
│                                                             │
├─────────────────────────────────────────────────────────────┤
│                    INTEGRATIONS                            │
│                                                             │
│  Stripe ────────────> Payment Processing                   │
│  Resend ─────────────> Email Notifications                │
│  M-Pesa ─────────────> Alternative Payments               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
         │                              │
         v                              v
   ┌──────────────┐          ┌──────────────────┐
   │ Stripe API   │          │ Webhook Events   │
   │              │          │ (Listener)       │
   │ • Payments   │          │                  │
   │ • Accounts   │          │ • Payment.success│
   │ • Products   │          │ • Account.update │
   │ • Connect    │          │ • Bank.created   │
   └──────────────┘          └──────────────────┘
\`\`\`

## 💳 Payment Flow Diagram

\`\`\`
Customer                Platform              Stripe              Seller
   │                       │                    │                   │
   │  1. Browse Products   │                    │                   │
   ├──────────────────────>│                    │                   │
   │                       │                    │                   │
   │  2. Click "Buy Now"   │                    │                   │
   ├──────────────────────>│                    │                   │
   │                       │                    │                   │
   │                       │ 3. Create Session  │                   │
   │                       ├───────────────────>│                   │
   │<──────────────────────┼────────────────────┤                   │
   │  Stripe Checkout URL  │                    │                   │
   │                       │                    │                   │
   │ 4. Enter Payment Info │                    │                   │
   ├──────────────────────────────────────────>│                   │
   │                       │                    │                   │
   │                       │ 5. Process Payment │                   │
   │                       ├───────────────────>│                   │
   │                       │<───────────────────┤                   │
   │                       │  Success Response  │                   │
   │                       │                    │                   │
   │ 6. Payment Success    │                    │ 7. Funds Transfer │
   │<──────────────────────┤────────────────────┼──────────────────>│
   │                       │                    │                   │
   │                       │ 8. Webhook Event   │                   │
   │                       │<───────────────────┤                   │
   │                       │                    │                   │
   │ 9. Confirmation Email │                    │                   │
   │<──────────────────────┤                    │                   │
   │  (via Resend)         │                    │ 10. Seller Notified
   │                       │                    │<───────────────────┤
   │                       │                    │                   │
\`\`\`

## 🔗 Endpoint Request/Response Flow

\`\`\`
┌──────────────────────────────────────────────────────────────┐
│ 1. BASIC CHECKOUT FLOW                                       │
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  Request:  POST /api/stripe/checkout                        │
│  ├─ amount: 9999 (cents)                                    │
│  ├─ currency: "usd"                                         │
│  ├─ customerEmail: "customer@email.com"                     │
│  └─ description: "Product Name"                             │
│                                                              │
│  Processing:                                                │
│  ├─ Validate config (STRIPE_SECRET_KEY exists)             │
│  ├─ Validate currency (usd, eur, gbp, kes)                 │
│  ├─ Validate amount (min $0.50)                            │
│  └─ Create Stripe session                                   │
│                                                              │
│  Response: 200 OK                                           │
│  └─ { success: true, url: "https://checkout.stripe.com..." }
│                                                              │
└──────────────────────────────────────────────────────────────┘
\`\`\`

\`\`\`
┌──────────────────────────────────────────────────────────────┐
│ 2. STRIPE CONNECT FLOW                                       │
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  Step 1: Create Connected Account                           │
│  Request:  POST /api/stripe/connect/create-account          │
│  ├─ displayName: "Business Name"                            │
│  ├─ contactEmail: "owner@business.com"                      │
│  └─ country: "us"                                           │
│  Response: { accountId: "acct_1234567890" }                 │
│                                                              │
│  Step 2: Generate Onboarding Link                           │
│  Request:  POST /api/stripe/connect/account-link            │
│  ├─ accountId: "acct_1234567890"                            │
│  Response: { url: "https://connect.stripe.com/..." }        │
│                                                              │
│  Step 3: Send Link to Seller                                │
│  └─ Seller clicks link and completes onboarding            │
│                                                              │
│  Step 4: Check Onboarding Status                            │
│  Request:  GET /api/stripe/connect/account-status?accountId │
│  Response: {                                                │
│    status: "complete"|"pending"|"incomplete",               │
│    onboardingComplete: true,                                │
│    readyForPayments: true,                                  │
│    requirements: { currentlyDue, pastDue, ... }             │
│  }                                                          │
│                                                              │
└──────────────────────────────────────────────────────────────┘
\`\`\`

\`\`\`
┌──────────────────────────────────────────────────────────────┐
│ 3. PRODUCTS API FLOW                                         │
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  Create Product:                                            │
│  Request:  POST /api/stripe/products                        │
│  ├─ name: "Premium Coffee"                                  │
│  ├─ description: "Freshly roasted"                          │
│  ├─ priceInCents: 1299                                      │
│  ├─ currency: "usd"                                         │
│  └─ accountId: "acct_1234567890" (seller)                   │
│  Response: { productId: "prod_123...", price: 1299 }        │
│                                                              │
│  List Products:                                             │
│  Request:  GET /api/stripe/products                         │
│  Response: { products: [...], total: 42 }                   │
│                                                              │
└──────────────────────────────────────────────────────────────┘
\`\`\`

\`\`\`
┌──────────────────────────────────────────────────────────────┐
│ 4. WEBHOOK FLOW                                              │
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  Stripe Event Occurs                                        │
│         │                                                   │
│         └──> Stripe sends POST to /api/webhooks/stripe      │
│                                                              │
│  Your Server:                                               │
│  ├─ Extract signature from headers                          │
│  ├─ Verify signature (STRIPE_WEBHOOK_SECRET)               │
│  ├─ Parse event JSON                                        │
│  ├─ Route based on event.type                               │
│  └─ Handle specific event                                   │
│                                                              │
│  Supported Events:                                          │
│  ├─ checkout.session.completed ──> Update order status     │
│  ├─ payment_intent.succeeded ───> Log payment              │
│  ├─ account.updated ────────────> Check requirements       │
│  └─ account.external_account.created ──> Enable payouts    │
│                                                              │
│  Response: { success: true, processed: "event_type" }       │
│                                                              │
└──────────────────────────────────────────────────────────────┘
\`\`\`

## 🔐 Security Validation Layers

\`\`\`
Request Received
      │
      v
┌──────────────────────────────┐
│ 1. ENV VALIDATION            │
│ ├─ STRIPE_SECRET_KEY exists? │
│ ├─ STRIPE_PUBLISHABLE_KEY?   │
│ └─ STRIPE_WEBHOOK_SECRET?    │
└──────────────────────────────┘
      │
      v (If Webhook)
┌──────────────────────────────────────┐
│ 2. SIGNATURE VERIFICATION            │
│ ├─ Extract signature from headers    │
│ ├─ Construct signed event            │
│ ├─ Compare signatures                │
│ └─ Accept/Reject based on match      │
└──────────────────────────────────────┘
      │
      v
┌──────────────────────────────┐
│ 3. INPUT VALIDATION          │
│ ├─ Email format check        │
│ ├─ Amount validation          │
│ ├─ Currency validation        │
│ └─ Required fields check      │
└──────────────────────────────┘
      │
      v
┌──────────────────────────────┐
│ 4. API CALL EXECUTION        │
│ ├─ Use Stripe client         │
│ ├─ Handle API errors         │
│ └─ Return safe response      │
└──────────────────────────────┘
      │
      v
┌──────────────────────────────┐
│ 5. ERROR HANDLING            │
│ ├─ Catch exceptions          │
│ ├─ Log for debugging         │
│ ├─ Return safe error message │
│ └─ Never expose sensitive    │
│    data in response          │
└──────────────────────────────┘
\`\`\`

## 📊 File Structure

\`\`\`
Your App
├── app/
│   ├── api/
│   │   ├── stripe/
│   │   │   ├── checkout/
│   │   │   │   └── route.ts        ✅ One-time payments
│   │   │   ├── products/
│   │   │   │   └── route.ts        ✅ Product management
│   │   │   └── connect/
│   │   │       ├── create-account/
│   │   │       │   └── route.ts    ✅ Create seller
│   │   │       ├── account-link/
│   │   │       │   └── route.ts    ✅ Onboarding link
│   │   │       └── account-status/
│   │   │           └── route.ts    ✅ Check status
│   │   └── webhooks/
│   │       └── stripe/
│   │           └── route.ts        ✅ Handle events
│   ├── layout.tsx
│   ├── page.tsx
│   └── globals.css
│
├── lib/
│   ├── stripe-config.ts            ✅ NEW - Config & validation
│   ├── email-service.tsx           ✅ Resend integration
│   └── mpesa.ts                    ✅ M-Pesa integration
│
├── components/
│   ├── payment-methods-section.tsx
│   └── ... other components
│
├── Documentation/
│   ├── STRIPE_DOCUMENTATION_INDEX.md
│   ├── STRIPE_QUICK_REFERENCE.md
│   ├── STRIPE_SETUP_GUIDE.md
│   ├── NETLIFY_ENV_VARIABLES.md
│   ├── STRIPE_COMPLETE_IMPLEMENTATION.md
│   ├── STRIPE_PRODUCTION_READY.md
│   └── START_HERE_STRIPE.md
│
└── .env.example                    ✅ Updated
\`\`\`

## 🧪 Test Scenarios

\`\`\`
┌─────────────────────────────────────────┐
│ SCENARIO 1: Successful Payment          │
├─────────────────────────────────────────┤
│                                         │
│ Input: Card 4242 4242 4242 4242        │
│ Amount: $99.99                          │
│ Currency: USD                           │
│                                         │
│ Expected Flow:                          │
│ ✅ Session created                      │
│ ✅ User redirected to checkout          │
│ ✅ Payment processed                    │
│ ✅ Webhook fired (session.completed)    │
│ ✅ Order updated                        │
│ ✅ Email sent                           │
│                                         │
└─────────────────────────────────────────┘

┌─────────────────────────────────────────┐
│ SCENARIO 2: Seller Onboarding           │
├─────────────────────────────────────────┤
│                                         │
│ Step 1: Create account                  │
│ ✅ Account created (acct_...)           │
│                                         │
│ Step 2: Get onboarding link             │
│ ✅ Link generated                       │
│                                         │
│ Step 3: Seller completes onboarding     │
│ ✅ Requirements submitted               │
│                                         │
│ Step 4: Check status                    │
│ ✅ Status shows "complete"              │
│ ✅ Ready for payments: true             │
│                                         │
└─────────────────────────────────────────┘

┌─────────────────────────────────────────┐
│ SCENARIO 3: Webhook Event               │
├─────────────────────────────────────────┤
│                                         │
│ Input: checkout.session.completed       │
│                                         │
│ Processing:                             │
│ ✅ Signature verified                   │
│ ✅ Event parsed                         │
│ ✅ Handler executed                     │
│ ✅ TODO tasks updated                   │
│                                         │
│ Response: 200 OK                        │
│                                         │
└─────────────────────────────────────────┘
\`\`\`

---

This provides a complete visual overview of how Stripe integrates with your platform!
